...
class NBV_Net_new(nn.Module):
    
    def __init__(self, dropout_prob):
        
        super(NBV_Net_new, self).__init__()
...
class NBV_Net_1FC(NBV_Net_new):
    def __init__(self, dropout_prob):
        NBV_Net_new.__init__(self, dropout_prob)
        self.fc1 = nn.Linear(512, 14)
    
    def forward(self, x):
        x = NBV_Net_new.forward(self, x)
        x = F.relu(self.fc1(x))
        x = F.softmax(x, dim=1)
        return x


class NBV_Net_2FC(NBV_Net_new):
    def __init__(self, dropout_prob):
        NBV_Net_new.__init__(self, dropout_prob)
